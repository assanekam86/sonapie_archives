...
<div class="card shadow mt-4">
    <div class="card-header py-3">
      <h1 class="m-0 font-weight-bold text-primary">LISTE DES PIECES</h1>
  <?php //var_dump($userss); ?>
       </div>
       <div class="card-body">
       <div class="table-responsive">
<table class="table table-bordered table-striped table-hover" data-order="[[ 11, &quot;desc&quot; ]]" id="dataTable" width="" cellspacing="0">
<thead> 
		<tr>
        <th>N°</th>
		<th>Référence Pièce</th>
	...
	<tbody>
		<?php 
        if($_SESSION['user']['roles'] =='ROLE_USER'){ 
        $user = new UsersModel;
        $userss = $user->requete('SELECT rl_types.id_type FROM users,categories,rl_types WHERE users.id = rl_types.id_user AND categories.id = rl_types.id_type AND users.id=? GROUP BY rl_types.id_type',[$_SESSION['user']['id']])->fetchAll();
      //  var_dump($userss); exit;
		foreach ($userss as $users) :
		...
		$afficheDoc = $docModel->requete(" SELECT * FROM boites as b, villes as v, rayons as ray, etageres as etag,salles as sal ,users as u ,rl_types as r,types as s,categories as c, dossiers as o, usagers as usag, documents as d WHERE b.id=o.id_boite AND ray.id=etag.id_rayon AND b.id_etagere=etag.id AND sal.id = ray.id_salle AND v.id=sal.id_ville AND usag.id=d.id_usager AND o.id=d.id_dos AND c.id=d.id_cat AND r.id_type = d.id_cat AND c.id=r.id_type AND c.id=s.id_cat AND s.id=d.id_types AND u.id = d.id_user AND d.actif=? AND d.id_cat=? GROUP BY d.reference ORDER BY d.date_creation_doc DESC",[1,$users->id_type])->fetchAll();
		
       /* $afficheDoc = $docModel->requete("SELECT * FROM 
villes as v,salles as s,rayons as ray,etageres as et,boites as b,usagers as u,types as t,dossiers as o,rl_types as r, categories as c,documents as d
WHERE
b.id=o.id_boite AND et.id=b.id_etagere AND ray.id=et.id_rayon AND s.id=ray.id_salle AND v.id=s.id_ville AND
u.id=d.id_usager AND t.id=d.id_types AND o.id=d.id_dos AND r.id_type=d.id_cat AND c.id=d.id_cat AND d.actif=? AND d.id_cat=? ORDER BY d.date_creation_doc DESC",[1,$users->id_type])->fetchAll();
        */
		
		
			//$docModel = new DocumentsModel;
		//$afficheDoc = $docModel->requete("SELECT * FROM boites as b, villes as v, rayons as ray, etageres as etag,salles as sal ,users as u ,types as s,categories as c, dossiers as o, usagers as usag, documents as d WHERE b.id=o.id_boite AND ray.id=etag.id_rayon AND b.id_etagere=etag.id AND sal.id = ray.id_salle AND v.id=sal.id_ville AND usag.id=d.id_usager AND o.id=d.id_dos AND c.id=d.id_cat AND c.id = d.id_cat AND c.id=s.id_cat AND s.id=d.id_types AND u.id = d.id_user AND d.actif=? GROUP BY d.reference ORDER BY d.date_creation_doc DESC",[1])->fetchAll();
		//}
		//$afficheFile = $docModel->requete("SELECT * FROM documents as d,fichiers as f WHERE d.id = f.id_doc AND d.id=? AND d.actif=? AND f.actif=? AND d.id_serv=? ORDER BY d.date_creation_doc DESC",[$id,1,1,$users->id_service])->fetchAll();
		//var_dump($afficheDoc);exit;
	
?>
		<?php $n=1; foreach ($afficheDoc as $files ): ?>
			...
			<tr>
				...
				<?php if(isset($_SESSION['user']['id']) && ($_SESSION['user']['roles']=='ROLE_ADMIN' || $_SESSION['user']['roles']=='ROLE_SUPERUSER') ){ ?>
						<div class="dropdown-item"><a href="/documents/modifier/<?=$files->id;?>" title="Modifier le dossier" class="btn btn-primary">
						<i class="far fa-edit"></i>
					</a></div>
						
					<div class="dropdown-item"><a href="/documents/supprimeDocument/<?=$files->id;?>" title="Supprimer le dossier" class="btn btn-danger">
...
<script type="text/javascript">
	$('#frame').click(function(){
		$('.container').printThis({
			...
			importStyle:false,
			printIframe: true,
			loadCSS:"http:archives.local/css/style.css",
			array []
			pageTitle:"Copie",
			removeInline:false,
			printDelay:333,
			header: "<h1 class='text-center'>Copie</h1>",
			footer:null,
			formvalue:true,
			canvas: false,
			base:false,
